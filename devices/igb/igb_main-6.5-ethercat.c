...
static bool igb_clean_tx_irq(struct igb_q_vector *q_vector, int napi_budget)
{
	...
	if (test_bit(__IGB_DOWN, &adapter->state)){
		//printk("igb_clean_tx_irq  __IGB_DOWN");
		return true;
	}
	...
	do {
		...
		if (!eop_desc){
			//printk("igb_clean_tx_irq  !eop_desc");
			break;
		}
		...
		if (!(eop_desc->wb.status & cpu_to_le32(E1000_TXD_STAT_DD))){
			//printk("igb_clean_tx_irq  !(eop_desc->wb.status & cpu_to_le32(E1000_TXD_STAT_DD)");
			break;
		}
			    ...
			    unsigned int size)
{
	...
	//printk(" #if (PAGE_SIZE < 8192) i = %d , truesize = %d",skb_shinfo(skb)->nr_frags,truesize);
#else
	unsigned int truesize = ring_uses_build_skb(rx_ring) ?
				SKB_DATA_ALIGN(IGB_SKB_PAD + size) :
				SKB_DATA_ALIGN(size);
	//printk(" i = %d , truesize = %d",skb_shinfo(skb)->nr_frags,truesize);
#endif
	//printk("i = %d , truesize = %d",skb_shinfo(skb)->nr_frags,truesize);			
