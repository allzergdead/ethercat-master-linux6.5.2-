...
//#include "r8169-6.5-ethercat.h"
//#include "r8169_firmware-6.5-ethercat.h"
...
/* 自定义锁定特定的内存页面 */
int my_lock_page(struct page *page)
{
    if (!page)
        return -EINVAL;
...
/* 自定义解除特定的内存页面锁定 */
void my_unlock_page(struct page *page)
{
    if (!page)
        return;
...
/* 锁定特定内存区域的所有页面 */
int lock_memory_range(unsigned long start, unsigned long end)
{
    unsigned long addr;
    int ret = 0;
...
/* 解除特定内存区域的所有页面锁定 */
void unlock_memory_range(unsigned long start, unsigned long end)
{
    unsigned long addr;
...
/* 锁定内核进程的所有内存 */
int lock_kernel_process_memory(struct task_struct *task)
{
    ...
    for (vma = find_vma(mm, 0); vma; vma = find_vma(mm, vma->vm_end)) {
        ...
        ret = lock_memory_range(start, end);
        if (ret) {
            pr_err("Failed to lock memory range: %d\n", ret);
...
/* 解除内核进程的所有内存页面锁定 */
void unlock_kernel_process_memory(struct task_struct *task)
{
    struct mm_struct *mm = task->mm;
    struct vm_area_struct *vma;
    unsigned long start, end;
    ...
    for (vma = find_vma(mm, 0); vma; vma = find_vma(mm, vma->vm_end)) {
        ...
        unlock_memory_range(start, end);
    }
    up_read(&mm->mmap_lock);
}


int set_cpus_chang(void){
    return 0;
    struct cpumask mask;
    ...
    //清空并设置CPU亲和性掩码
    cpumask_clear(&mask);
    cpumask_set_cpu(0, &mask);  // 将新线程绑定到
    cpumask_set_cpu(1, &mask);  // 将新线程绑定到
    //cpumask_set_cpu(2, &mask);  // 将新线程绑定到
    cpumask_set_cpu(3, &mask);  // 将新线程绑定到
    cpumask_set_cpu(4, &mask);  // 将新线程绑定到
    cpumask_set_cpu(5, &mask);  // 将新线程绑定到
    //设置新线程的CPU亲和性
...
static int rtl_rx(struct net_device *dev, struct rtl8169_private *tp, int budget)
{
	...
	for (count = 0; count < budget; count++, tp->cur_rx++) {
		...
		if (tp->ecdev) {
			...
			//printk("ecdev_receive pkt_size %d \n",pkt_size);
...
{
	//printk("ec_poll step %d \n",0);
	struct rtl8169_private *tp = netdev_priv(dev);
	u16 status;

	status = rtl_get_events(tp);
	
	//printk("ec_poll status %d \n",status);
	gvlSlow = SYSErr | RxOverflow | RxFIFOOver;
	rtl_ack_events(tp, status & ~gvlSlow);
	...
	if (jiffies - tp->ec_watchdog_jiffies >= 2 * HZ) {
		...
		bool link = netif_carrier_ok(dev);
		ecdev_set_link(tp->ecdev, link);
...
static void r8169_phylink_handler(struct net_device *ndev)
{
	...
	if (tp->ecdev) {
		bool link = netif_carrier_ok(ndev);
		ecdev_set_link(tp->ecdev, link);
		return;
	}
...
static int rtl_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
    ...
    //  }
	set_cpus_chang();
	printk("ecdev_receive init dev step %d \n",1);
	printk("ecdev_receive init dev step %d \n",11);
	...
	if (tp->dash_type != RTL_DASH_NONE) {
		...
		printk("ecdev_receive DASH enabled no rtl8168_driver_start %d \n",0);
	}
	printk("ecdev_receive init dev step %d \n",5);
	if (pci_dev_run_wake(pdev))
		pm_runtime_put_sync(&pdev->dev);
